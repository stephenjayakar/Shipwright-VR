...
// These may already be defined as macros by the game's GBI header.
...
public:
    ...
    // Register a pre-decoded RGBA texture with the texture manager.
    // This is the primary entry point for the N64 texture pipeline integration.
    // Returns the SRV index (>0 on success, 0 on failure).
    // Parameters:
    //   hash     - Unique hash key for cache lookup/dedup
    //   width    - Texture width in texels
    //   height   - Texture height in texels
    //   rgbaData - Pre-decoded RGBA8 pixel data (4 bytes per pixel)
    uint32_t RegisterTexture(uint64_t hash, uint32_t width, uint32_t height, const uint8_t* rgbaData);
    ...
    // Register an alias hash that maps to the same SRV index as an existing texture.
    // This is used so that textures uploaded under a full hash (address+width+height+format)
    // can also be looked up by an address-only hash (used during geometry extraction).
    void RegisterHashAlias(uint64_t aliasHash, uint32_t srvIndex);
    ...
    // Eagerly resolve a material texture by OTR path string.
    // This attempts to load the texture from the OTR archive, decode it,
    // upload it to the GPU, and register it under all relevant hash aliases.
    // Returns the SRV index (>0 on success, 0 on failure).
    // Called from SceneGeometryExtractor::GetOrCreateMaterial to resolve
    // textures at extraction time rather than waiting for deferred resolution.
    // Parameters:
    //   otrPath    - OTR path string (e.g., "__OTR__scenes/shared/...")
    //   texFormat  - N64 image format (G_IM_FMT_*: 0=RGBA, 2=CI, 3=IA, 4=I)
    //   texSize    - N64 image size (G_IM_SIZ_*: 0=4b, 1=8b, 2=16b, 3=32b)
    //   texWidth   - Texture width in texels
    //   texHeight  - Texture height in texels
    //   tlutPath   - OTR path for CI palette (TLUT), nullptr if not CI format
    uint32_t EagerResolveOTRTexture(const char* otrPath,
                                     uint8_t texFormat, uint8_t texSize,
                                     uint16_t texWidth, uint16_t texHeight,
                                     const char* tlutPath = nullptr);

    // Get a diagnostic summary string of current texture cache state.
    // Used for RTX_DIAG logging.
    void LogCacheStats() const;

    // Log TEXTURE_BIND diagnostics for all textures bound to geometry pieces.
    // Only logs on the first call (first frame). Subsequent calls are no-ops.
    // This helps debug the Deku Tree white texture issue by showing exactly which
    // SRV index, dimensions, and format each geometry material is using.
    //
    // Parameters:
    //   materials     - Array of Material structs from an ExtractedMesh
    //   materialCount - Number of materials in the array
    //   meshName      - Human-readable mesh name for log output (e.g., "opaque", "alpha")
    //   roomIndex     - Room index for log context
    void LogFirstFrameTextureBinds(const Material* materials, uint32_t materialCount,
                                    const char* meshName, uint32_t roomIndex);

    // Check if first-frame texture bind logging has already been done.
    bool HasLoggedFirstFrameBinds() const { return mFirstFrameBindLogged; }
...
private:
    ...
    // Alias hash -> SRV index map.  Allows looking up textures by alternative
    // hash keys (e.g. address-only hash when the primary key is address+dims+format).
    std::unordered_map<uint64_t, uint32_t>         mHashAliases;
    ...
    // First-frame texture bind logging flag — used to emit TEXTURE_BIND diagnostics
    // exactly once (on the first frame after initialization) to help debug which
    // textures are bound to which geometry. Set to true after the first frame's
    // logging pass completes.
    bool                                           mFirstFrameBindLogged = false;
};
