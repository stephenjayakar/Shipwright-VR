...
// Scene constants are declared in Common.hlsli as cbuffer SceneConstants : register(b0)
...
float3 SpatialFilter3x3(uint2 pixel, uint2 dims) {
    ...
    float sigma = giIntensity > 0.0 ? 0.15 / max(giIntensity, 0.1) : 0.15;
...
void GlobalIllumination(uint3 DTid : SV_DispatchThreadID) {
    ...
    float blendFactor = ComputeTemporalBlendFactor(frameCount);
    ...
    // reset accumulation.
    float3 blended;
    if (frameCount == 0 || prevWeight < 0.001) {
    ...
    if (aoIntensity > 0.0) {
        float aoFactor = 1.0 - aoIntensity * 0.15;
        blended *= aoFactor;
    }
