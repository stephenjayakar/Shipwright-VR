...
| Phase 7: Scene Integration Hooks | **DONE** (enhanced) | RTXHooks.h/cpp + RTXRenderer.h/cpp orchestrator. 2026-02-08: Added two-phase init (probe DXR before DX11 teardown) + safety guard for failed init. |
...
## Current Status & Debug Findings (Feb 8, 2026)

### Overall Status: Screen is BLACK — geometry extraction yields 0 vertices

The DX12 device, pipeline, shaders, and acceleration structures all initialize
successfully.  The pipeline state object is created, shaders load from
precompiled `.cso` files, and the DXR pipeline dispatches rays.  However the
output is black because **no geometry is extracted** and therefore the TLAS is
empty — all rays miss.

### Root Causes Identified

#### BUG 1 (CRITICAL — ROOT CAUSE): Display-list pointers are OTR path strings, not Gfx data

**File:** `SceneGeometryExtractor.cpp` — `ExtractRoomGeometry()`

In the SoH (OTR) build the `PolygonDlist::opa` / `PolygonDlist2::opa` (and
`xlu`) fields are **not** pointers to Gfx command data.  They are `char*`
pointers to OTR resource path strings such as
`"__OTR__scenes/shared/spot04_scene/spot04_room_0DL_008ED8"`.

This is set in `soh/soh/resource/importer/scenecommand/SetMeshFactory.cpp`
lines 148-154:
...
Because `SEGMENTED_TO_VIRTUAL(addr)` is a no-op macro (`#define
SEGMENTED_TO_VIRTUAL(addr) addr` in `soh/include/macros.h`), our code was
doing:

```cpp
Gfx* opaDL = (Gfx*)SEGMENTED_TO_VIRTUAL(dlists[i].opa);
WalkDisplayList(opaDL, ...);  // walks a C string as Gfx commands!
```

This caused the geometry extractor to interpret ASCII path characters as GBI
opcodes and operands, producing garbage hashes, zero vertices, and access
violations.

**Fix (IN PROGRESS):** Call `ResourceMgr_LoadGfxByName((const char*)dlists[i].opa)`
to resolve the OTR path to the actual `Gfx*` display-list pointer before
walking it.  A `resolveOTRDL()` helper lambda detects `"__"` prefix strings
and resolves them.

Evidence from the log that confirms this is the root cause:
- Hash `0x6E6573735F343074` = ASCII `"ness_40t"` — a fragment of an OTR path
  being read as a 64-bit CRC hash
- All `G_DL_OTR_HASH` failures show the same pattern of path-fragment "hashes"
- Access violations at depth 0 when the SEH wrapper catches a crash from
  walking random memory as a display list

#### BUG 2: UAV descriptor heap ordering — TextureManager not initialized yet

**File:** `DXRPipeline.cpp` — `CreateOutputBuffers()`, line ~529 in log

```
[RTX] TextureManager SRV heap not available — UAV descriptors not created
```

The initialization order in `RTXRenderer::Initialize()` is:
1. `m_pipeline->Initialize()` → calls `CreateOutputBuffers()` → tries to get
   `TextureManager::GetInstance().GetSRVHeap()` → **returns null** (not init'd)
2. `m_accelerationStructure->Initialize()`
3. `m_giSystem->Initialize()`
4. `TextureManager::GetInstance().Initialize()` — **too late!**

Because the UAV descriptors are never created, `DispatchRays()` binds a null
descriptor table for the output UAVs, so even if geometry existed, the output
would still be black.

**Fix (TODO):** Extract UAV descriptor creation from `CreateOutputBuffers()`
into a separate `CreateUAVDescriptors()` method.  Call it from
`RTXRenderer::Initialize()` **after** `TextureManager::Initialize()`.

#### BUG 3 (FIXED): Denoise root signature used root UAVs with RWTexture2D

Root UAV descriptors (`SetComputeRootUnorderedAccessView`) only work with
raw/structured buffers, NOT `RWTexture2D`.  The denoise compute shader declares
`RWTexture2D<float4>` for u0 and u1.

**Fix (DONE):** Changed denoise root signature from 3 params (2 root UAVs + CBV)
to 2 params (1 descriptor table with u0+u1 range + 32-bit constants).  Created
contiguous descriptor pairs in the TextureManager heap for ping-pong:
- Indices 4087-4088: even pass (u0=output, u1=temp)
- Indices 4089-4090: odd pass  (u0=temp, u1=output)

#### BUG 4 (FIXED): Raytracing pipeline root signature UAVs with RWTexture2D

Same root UAV vs descriptor table issue as the denoise pipeline, but for the
raytracing global root signature.

**Fix (DONE):** Changed root params [2]+[3] from individual root UAVs to a
single `D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE` with a UAV range of 2
(u0+u1).  UAV descriptors placed at heap indices 4093-4095.

#### BUG 5 (FIXED): OTR hash assembly was using wrong word fields

The RTX code originally assembled 64-bit CRC hashes from the wrong Gfx words
for `G_DL_OTR_HASH`, `G_VTX_OTR_HASH`, and `G_SETTIMG_OTR_HASH` commands.
Fixed to read from `cmd[1].w0` (high) and `cmd[1].w1` (low), matching the
Fast3D interpreter at `libultraship/src/fast/interpreter.cpp:3102`.

#### BUG 6 (FIXED): SEH wrapper crash in RTX_AcquireWindowForDX12

`SDL_GetGrabbedWindow_REAL` access violation during DXGI swap chain teardown.
Fixed with SEH + Win32 `GetActiveWindow()` / `GetClientRect()` fallback.

### What To Do Next (Priority Order)

1. **Fix BUG 1 — OTR DL path resolution** (IN PROGRESS in SceneGeometryExtractor.cpp)
   - The `resolveOTRDL()` lambda is written but needs build+test
   - Also applies to `G_DL_OTR_FILEPATH` handler at line 288 — verify it still works

2. **Fix BUG 2 — UAV descriptor ordering**
   - Add `DXRPipeline::CreateUAVDescriptors()` public method
   - Move descriptor creation logic from `CreateOutputBuffers()` into it
   - Call from `RTXRenderer::Initialize()` after `TextureManager::Initialize()`

3. **Build and test** — `cmake --build build --target soh --config Release`
   - Verify geometry extraction succeeds (log should show non-zero verts/tris)
   - Verify UAV descriptors are created (log should show heap indices)
   - Verify DispatchRays produces non-black output

4. **If hashes still fail inside the now-correctly-resolved display lists:**
   - The DL data loaded by `ResourceMgr_LoadGfxByName()` contains OTR-format
     commands (`G_DL_OTR_HASH`, `G_VTX_OTR_HASH`, etc.) with CRC hashes
   - Verify the CRC hashes resolve via `ResourceMgr_LoadGfxByCRC()` /
     `ResourceMgr_LoadVtxByCRC()`
   - If not, may need to use `GetResourceRawPointer(hash)` directly instead,
     which is what the Fast3D interpreter uses

5. **DX11 swap chain conflict** — If the SEH fallback path triggers, the DX11
   swap chain is not released and DX12 swap chain creation will fail.  May need
   a child/overlay window or offscreen DX12 approach.

### Files Modified (Complete List)

| File | Changes |
|------|---------|
| `soh/CMakeLists.txt:22` | `ENABLE_DX12_RTX` set to `ON` |
| `soh/src/code/z_scene_table.c` | RTX hook in Kokiri Forest draw config |
| `soh/src/code/z_play.c` | `RTX_OnSceneUnload()`, Room_Draw suppression |
| `soh/soh/OTRGlobals.cpp` | `RTX_AcquireWindowForDX12()`, `RTX_GetWindowDimensions()`, dispatch/shutdown hooks |
| `soh/soh/z_scene_otr.cpp` | `RTX_OnRoomLoaded()` hook |
| `soh/soh/z_play_otr.cpp` | `RTX_OnSceneLoaded()` hook |
| `soh/soh/Enhancements/RTX/RTXHooks.h` | Added declarations + no-op stubs |
| `soh/soh/Enhancements/RTX/RTXHooks.cpp` | SEH-wrapped `TryLazyInitialize` with Win32 fallback |
| `soh/soh/Enhancements/RTX/RTXRenderer.cpp` | `IsActive()`, `IsRTXSceneActive()` |
| `soh/soh/Enhancements/RTX/RTXRenderer.h` | Added `IsRTXSceneActive()` declaration |
| `soh/soh/Enhancements/RTX/DXRPipeline.h` | Changed `GlobalRootParam` enum (UAVTable), added `UAV_DESCRIPTOR_START`, denoise ping-pong constants + handles |
| `soh/soh/Enhancements/RTX/DXRPipeline.cpp` | Root sig UAV→descriptor table, UAV descriptors in TextureManager heap, `DispatchRays` uses descriptor table, denoise root sig + dispatch fixed |
| `soh/soh/Enhancements/RTX/SceneGeometryExtractor.cpp` | Fixed hash assembly for G_DL/VTX/SETTIMG_OTR_HASH; vertex byte offset; SEH wrapper; OTR path resolution (IN PROGRESS) |
| `soh/soh/Enhancements/RTX/SceneGeometryExtractor.h` | Added `WalkDisplayListInner` declaration |
| `libultraship/include/fast/backends/gfx_dxgi.h` | Added `ReleaseSwapChain()` |
| `libultraship/src/fast/backends/gfx_dxgi.cpp` | Implemented `ReleaseSwapChain()` |

### Key Reference: OTR Display List Data Flow

Understanding the full data flow is critical for the geometry extractor:

```
OTR Archive (on disk)
  └─ Binary display list data: opcodes + CRC hashes
       Written by OTRExporter/DisplayListExporter.cpp
       G_DL → G_DL_OTR_HASH (opcode 0x31)
       G_VTX → G_VTX_OTR_HASH (opcode 0x32)
       G_SETTIMG → G_SETTIMG_OTR_HASH (opcode 0x20)
       128-bit format: cmd[0] = opcode|flags + metadata
                       cmd[1] = CRC64(resourcePath) split as w0=high, w1=low

DisplayListFactory (binary reader)
  └─ Reads 8 bytes per Gfx word, detects "expanded" opcodes, reads extra 8 bytes
     Stores in Instructions vector as 2 Gfx entries (cmd[0], cmd[1])
     NO patching — hash bytes stored as-is

ResourceMgr_LoadGfxByName("__OTR__scenes/shared/spot04_scene/spot04_room_0DL_008ED8")
  └─ Returns Gfx* = DisplayList::GetPointer() = Instructions.data()
     This Gfx* points to OTR-format commands (not standard N64 GBI)

Room mesh header (SetMeshFactory.cpp)
  └─ PolygonDlist2::opa = (Gfx*)pathString.c_str()  // NOT Gfx data!
     Must call ResourceMgr_LoadGfxByName() to get actual Gfx*

Fast3D interpreter (runtime rendering)
  └─ gfx_dl_otr_hash_handler_custom():
       Reads hash from cmd[1], calls GetResourceRawPointer(hash)
       Returns Gfx* to sub-display-list, recurses

Our geometry extractor (SceneGeometryExtractor.cpp)
  └─ Must do the same: read hash, call ResourceMgr_LoadGfxByCRC(hash)
     or GetResourceRawPointer(hash) to resolve sub-DLs
     Similarly for VTX: ResourceMgr_LoadVtxByCRC(hash)
     Similarly for SETTIMG: resolve texture hash to texture data
```
